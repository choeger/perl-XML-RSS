...
sub _out_single_media_content {
    ...
    if ($has_nested) {
        # Output as container with nested elements
        $self->_out("<media:content" . (@attrs ? " " . join(" ", @attrs) : "") . ">\n");
        
        # Output nested elements using helper method
        $self->_out_media_elements_collection($content);
        
        $self->_out("</media:content>\n");
    } else {
...
sub _out_media_elements_collection {
    ...
    for my $elem_name (@{MEDIA_NESTED_ELEMENTS()}) {
        ...
        if (grep { $_ eq $elem_name } @{MEDIA_SIMPLE_TEXT_ELEMENTS()}) {
            ...
            $self->_out("<media:$elem_name>" . $self->_encode($value) . "</media:$elem_name>\n");
        }
            ...
            if (ref($value) eq 'HASH') {
                ...
                $self->_out("<media:$elem_name" . (@attrs ? " " . join(" ", @attrs) : "") . "/>\n");
            }
        }
        else {
            ...
            if (ref($value) eq 'HASH') {
                ...
                $self->_out("<media:$elem_name$attrs>" . $self->_encode($element_content) . "</media:$elem_name>\n");
            } else {
                $self->_out("<media:$elem_name>" . $self->_encode($value) . "</media:$elem_name>\n");
            }
        }
...
sub _out_modules_elements {
    ...
    while (my ($url, $prefix) = each %{$self->_modules}) {
        next if $prefix =~ /^(dc|syn|taxo|media)$/;
