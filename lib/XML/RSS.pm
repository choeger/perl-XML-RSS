...
sub _handle_char {
    ...
    # item element
        ...
        if (!$elem_verdict && exists($self->{modules}->{$elem_ns}) &&
            $self->{modules}->{$elem_ns} eq MEDIA_PREFIX) {
            ...
            if ($parser->within_element($parser->generate_ns_name("content", MEDIA_RSS_NAMESPACE))) {
                ...
                my $content_ref = $item->{media}->{content};
            ...
            if ($self->_should_be_hashref($current_elem)) {
                ...
                my $item = $self->_last_item;
                if (exists($item->{media}->{$current_elem}) && ref($item->{media}->{$current_elem}) eq 'HASH') {
                    $item->{media}->{$current_elem}->{content} .= $cdata;
                }
                return;
            }
        }
    ...
    # channel element
        ...
        if (!$elem_verdict && exists($self->{modules}->{$elem_ns}) &&
            ...
            if ($self->_should_be_hashref($current_elem)) {
                # Store content in the hashref structure directly
                if (exists($self->{channel}->{media}->{$current_elem}) &&
                    ref($self->{channel}->{media}->{$current_elem}) eq 'HASH') {
                    $self->{channel}->{media}->{$current_elem}->{content} .= $cdata;
                }
                return;
            }
            # Handle simple media elements at channel level
            $self->{channel}->{media}->{$current_elem} .= $cdata;
            return;
        }
...
sub _should_be_hashref {
    ...
    if (!$el_verdict && exists($self->{modules}->{$el_ns}) &&
        $self->{modules}->{$el_ns} eq MEDIA_PREFIX) {
...
sub _handle_media_array_element {
    ...
    return unless $prefix eq MEDIA_PREFIX && ($el eq 'content' || $el eq 'group');
...
sub _handle_start {
        ...
        && $self->_current_element eq 'item')
    {
        ...
        if (keys(%attribs)) {
            my $group_ref = $self->_last_item->{media}->{group};
            ...
            if (ref($group_ref) eq 'ARRAY') {
                ...
                if ($el_verdict) {
                    ...
                    my $prefix = $self->{modules}->{$el_ns};
                    if ($prefix && $prefix eq MEDIA_PREFIX) {
        ...
        if (keys(%attribs)) {
            my $content_ref = $self->_last_item->{media}->{content};
